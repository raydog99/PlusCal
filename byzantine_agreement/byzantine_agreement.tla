...
\* Phase 2: Nodes exchange messages and reach agreement
Agree(p) ==
    /\ state[p].phase = "prepare"
    /\ LET agreed_value == CHOOSE v \in Value : \A q \in 1..N \ {p} : state[q].phase = "prepare" => state[q].x = v
       IN state' = [state EXCEPT ![p].phase = "commit", ![p].x = agreed_value]
