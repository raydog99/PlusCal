...
\* Phase 2: Nodes exchange messages and reach consensus with optimizations for practicality
FastPracticalAgree(p) ==
    /\ state[p].phase = "prepare"
    /\ LET Q == {q \in 1..N : state[q].phase = "prepare"}
       /\ LET v == CHOOSE v \in Value : \A q \in Q : state[q].x = v
